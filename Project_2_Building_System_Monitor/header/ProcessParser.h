...
string ProcessParser::getVmSize(string pid)
{
    ...
    while(getline(stream, line)){
        ...
        if (line.compare(0, name.size(),name) == 0) {
            ...
            istream_iterator<string> beg(buf), end; // Why did we add end here?
...
{
    std::vector<string> processList;
    processList = ProcessParser::getPidList();
    if (find(processList.begin(), processList.end(), pid) != processList.end())
        return true;
    else
        return false;
}
